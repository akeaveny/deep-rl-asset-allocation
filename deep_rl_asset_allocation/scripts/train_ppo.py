...
def train_PPO(env_train, model_name, timesteps=TIMESTEPS):
    """PPO model"""
    ppo_saved_model_filename = os.path.join(paths_config.results_trained_models_dir, f'{model_name}_{timesteps}')
    # print(f"Saved Model to {ppo_saved_model_filename}")

    start = time.time()
    model = PPO(MlpPolicy, env_train, ent_coef=0.005)  # nminibatches = 8
    model.learn(total_timesteps=timesteps)
    model.save(ppo_saved_model_filename)
    training_time = (time.time() - start) / 60
    print(f'Training time (PPO): {training_time:.1f} minutes')
    return model
...
def main():
    ...
    for training_iteration, idx in enumerate(range((rebalance_window + validation_window), len(unique_trade_dates), rebalance_window)):
        ...
        model_ppo = train_PPO(env_train, model_name=f"ppo_djia_training_iteration_{training_iteration}_timesteps", timesteps=TIMESTEPS)
        print(f"Validating PPO ...")
        validate_model(model=model_ppo, df_val=_df_val, env_val=env_val)
        print(f"Testing PPO ...")
        previous_observation = test_model(model=model_ppo, df_test=_df_test, env_test=env_test, training_iteration=training_iteration)
